...
//$totalscoreQ = mysql_query("SELECT (sum(exp(s1.score-s2.score))+exp(".$los."-s2.score)) AS score FROM shares_history s1, shares_history s2 WHERE s2.id = s1.id - 1 AND s1.counted = 0");
...
if ($settings->getsetting("siterewardtype") == 0)
{
	...
	$userListQ = mysql_query("SELECT id, shares_this_round, donate_percent from webUsers");
	while ($userListR = mysql_fetch_object($userListQ)) {
		$donatePercent = $userListR->donate_percent;
		$shareRatio = $userListR->shares_this_round/$totalRoundShares;
